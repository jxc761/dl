...
function build_ds()
   -- data configuration
  local input  = {dtype='image', ctype='gray', bidirect=false, res=32, step=1, stride=1, speed=1}
  local target = {dtype='depth', ctype='inverse', bidirect=false, res=32, step=1, stride=1, speed=1}
  local inputProc =  getPreproc('vectorize')
  local targetProc = getPreproc('vectorize')
